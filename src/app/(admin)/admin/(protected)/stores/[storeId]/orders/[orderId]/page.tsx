...
const REFUND_STATUS_STYLES: Record<RefundRequestStatus, string> = {
  pending:   "bg-yellow-100 text-yellow-700 dark:bg-yellow-900/30 dark:text-yellow-400",
                ...
                <ol className="relative border-l border-admin-border space-y-4 ml-2">
                  {[...order.statusHistory].reverse().map((entry, idx) => (
                    <li key={idx} className="ml-4">
                      <span
                        className={`absolute -left-1.5 w-3 h-3 rounded-full border-2 border-admin-surface ${
              ...
              {order.clientIp ? (
                sameIpOrders.length > 0 ? (
                  <div className="space-y-3">
                    <div className="flex items-center gap-2">
                      <span className="inline-flex items-center gap-1.5 text-xs font-medium bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400 px-2.5 py-1 rounded-full">
                    ...
                    <div className="rounded-lg border border-red-200 dark:border-red-900/50 overflow-hidden">
                      <div className="bg-red-50 dark:bg-red-900/20 px-4 py-2 text-xs font-semibold text-red-700 dark:text-red-400">
              ...
              headerRight={
                <span
                  className={`px-2.5 py-1 text-xs font-semibold rounded-full capitalize ${
                    REFUND_STATUS_STYLES[order.refundRequest.status]
                  }`}
                >
              ...
              {(order.refundRequest.status === "pending" ||
                ...
                <div className="px-5 py-4 space-y-2 text-sm">
                  ...
                  <p className="text-admin-text-muted">
                    ...
                    <span className="text-admin-text-secondary">{order.refundRequest.reason}</span>
                  </p>
                  {order.refundRequest.adminNote && (
                    <p className="text-admin-text-muted">
                      <span className="font-medium">Admin note: </span>
                      <span className="text-admin-text-secondary">{order.refundRequest.adminNote}</span>
                    </p>
                  )}
                  {order.refundRequest.processedAt && (
                    <p className="text-xs text-admin-text-subtle">
                      Processed{" "}
                      {new Date(order.refundRequest.processedAt).toLocaleString("en-GB", {
                        day: "2-digit",
                        month: "short",
                        year: "numeric",
                      })}
