...
export default async function CustomerPointsPage({
  ...
  const [summary, store] = await Promise.all([
    PointService.getCustomerSummary(storeId, userId, {
      page,
      limit: LEDGER_SIZE,
    }),
  ...
  return (
    <div>
      ...
      <nav className="flex items-center gap-1.5 text-sm text-admin-text-muted mb-3 flex-wrap">
        ...
        <ChevronRight className="w-3.5 h-3.5 shrink-0" />
        <Link href={`/admin/stores/${storeId}/points`} className="hover:text-admin-text-secondary transition-colors">Loyalty Points</Link>
        <ChevronRight className="w-3.5 h-3.5 shrink-0" />
        <span className="text-admin-text-secondary font-medium">{user.name}</span>
      </nav>
